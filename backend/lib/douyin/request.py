...
class Request(object):
    ...
    def __init__(self, cookie="", UA=""):
        ...
        # 代理（与浏览器一致时可减少接口空 body）：环境变量 HTTP_PROXY / HTTPS_PROXY
        self.proxies = None
        for key in ("HTTPS_PROXY", "https_proxy", "HTTP_PROXY", "http_proxy"):
            val = os.environ.get(key, "").strip()
            if val:
                self.proxies = {"http": val, "https": val}
                break
    ...
    def getHTML(self, url) -> str:
        ...
        response = requests.get(
            url, headers=headers, cookies=self.COOKIES, proxies=self.proxies, timeout=15
        )
    ...
    def getJSON(self, uri: str, params: dict, data: dict = None):
        ...
        if response.status_code != 200:
            logger.error(
                f"JSON请求失败：url: {url}, code: {response.status_code}, body: {response.text[:500]!r}"
            )
            return {}

        if not response.text or not response.text.strip():
            logger.error(
                "JSON请求失败：响应体为空。请尝试：1) 在设置中更新为最新的 Cookie；"
                "2) 在设置中填写 User-Agent（与获取 Cookie 的浏览器一致，如 Edge/Chrome）；3) 稍后重试（可能被限流）"
            )
            return {}
        ...
        try:
            ...
            logger.error(
                f"JSON请求失败：响应非 JSON，url: {url}, 前 300 字符: {response.text[:300]!r}, 解析错误: {e}"
            )
            return {}

        if resp_data.get("status_code", 0) != 0:
            api_msg = resp_data.get("message", resp_data.get("status_msg", ""))
            logger.error(
                f"接口返回错误：status_code={resp_data.get('status_code')}, message={api_msg!r}。"
                "若为登录/签名问题，请更新 Cookie 后重试"
            )
            return {}
