...
class DataParser:
    ...
    def _parse_single_aweme(item: dict, type: str) -> dict:
        ...
        # 统一点赞/评论/分享数（兼容 API 的 snake_case、camelCase 及顶层字段）
        def _num(key_snake: str, key_camel: str) -> int:
            v = aweme.get(key_snake) or aweme.get(key_camel)
            if v is not None:
                return int(v)
            v = item.get(key_snake) or item.get(key_camel)
            return int(v) if v is not None else 0

        aweme["digg_count"] = _num("digg_count", "diggCount")
        aweme["comment_count"] = _num("comment_count", "commentCount")
        aweme["share_count"] = _num("share_count", "shareCount")
